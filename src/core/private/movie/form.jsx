...
//           </button>
//           <button
//             onClick={() => handleDeleteMovie(row.original.id)}
...
//                 e.preventDefault();
//                 const movieData = {
//                   id: currentMovie?.id, // Only include ID if editing
...
const MovieAdmin = () => {
  ...
  // Fetch movies manually after mutation
  const fetchMovies = async () => {
    const response = await axios.get("http://localhost:4011/api/movie/get");
    setMovies(response.data);
  };

  // Use Mutation for adding and updating movies
  const saveMovieMutation = useMutation(
    (movieData) => {
      ...
      const apiUrl = movieData.id
        ? `http://localhost:4011/api/movie/${movieData.id}`
        : "http://localhost:4011/api/movie/save";

      return axios({
        method: movieData.id ? "put" : "post",
        url: apiUrl,
        data: formData,
        headers,
      });
    ...
    {
      onSuccess: () => {
        // Invalidate the query or update state here
        setIsModalOpen(false); // Close the modal
      },
    }
  );

  // Use Mutation for deleting a movie
  const deleteMovieMutation = useMutation(
    ...
    {
      onSuccess: () => {
        fetchMovies(); // Manually refetch the movies after deletion
      },
    }
  );
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow">
        <table className="table w-full">
          ...
          <tbody>
            ...
            {movies.length === 0 ? (
              <tr>
                <td colSpan={columns.length}>No movies available</td>
              </tr>
            ) : (
              movies.map((movie) => (
                <tr key={movie.id}>
          ...
          <div className="modal-box">
            ...
            <form
              ...
              onSubmit={(e) => {
                ...
                saveMovieMutation.mutate(movieData); // Use the mutation
              }}
            >
              {/* Form fields */}
              <div className="form-control">
