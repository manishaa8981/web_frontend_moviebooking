...
const MovieTicket = ({ movie, showtime, hall, seats, totalAmount }) => {
  return (
    <div className="max-w-sm mx-auto bg-white rounded-xl shadow-lg border border-gray-300 p-4 relative">
      {/* Left Perforation */}
      <div className="absolute left-0 top-1/2 transform -translate-y-1/2 w-5 h-5 bg-gray-200 rounded-full"></div>

      {/* Right Perforation */}
      <div className="absolute right-0 top-1/2 transform -translate-y-1/2 w-5 h-5 bg-gray-200 rounded-full"></div>

      {/* Ticket Header */}
      <div className="text-center border-b pb-2">
        <h2 className="text-xl font-bold text-gray-800">🎟 Movie Ticket</h2>
      </div>
      ...
      <div className="mt-4 text-left space-y-2">
        ...
        <p>
          <strong>Seats:</strong> 🎟 {seats.length} Selected [{seats.join(", ")}]
        </p>
      ...
      {/* Ticket Footer */}
      <div className="mt-4 border-t pt-2 text-center text-gray-500 text-sm">
        Enjoy your movie! 🍿
      </div>
    </div>
  );
...
export default function App() {
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <MovieTicket
        movie="Bhul Bhuliya 3"
        showtime="3rd March 2025"
        hall="Galaxy Theater"
        seats={["A4", "A5"]}
        totalAmount="500"
      />
